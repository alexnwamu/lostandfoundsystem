...
const ItemsFeed = ({ itemData,activeCategory }: Props) => {
    const filteredItems = activeCategory === 'All' ? itemData : itemData.filter(item => item.category === activeCategory);
 if (filteredItems.length >0){ return (
          ...
          <DialogContent className="max-w-[1110px] ">
            ...
            <div className="mt-[105px] flex gap-[66px]">
              ...
              <div className="text-[#615C61] text-[14px] ">
                <h2>
                  Found Date: <span className="ml-1">{item.dateCreated}</span>
                </h2>
                <h2>
            ...
            <div className="mt-[85px] w-full flex">
              <Link  className="  ml-auto mr-[158px] " href={{ pathname: "/contact", query: { tag: item.tag } }}>
    ...
    return(
    <div className="font-bold">No Items available</div>
    )
